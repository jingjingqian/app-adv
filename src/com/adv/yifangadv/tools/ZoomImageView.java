...
public class ZoomImageView extends ImageView {
	...
	protected void onDraw(Canvas canvas) {
		...
		default:
			if (sourceBitmap != null)
				clearCanvasAndDraw(canvas, sourceBitmap, matrix);
			break;
		}
	}
	...
	//清屏然后再画
	private void clearCanvasAndDraw(Canvas canvas, Bitmap bitmap, Matrix matrix)
	{
		if(canvas!=null){  
				canvas.save();//锁定画布
	            Paint paint = new Paint();  
	            paint.setXfermode(new PorterDuffXfermode(Mode.SRC));
	            paint.setColor(Color.parseColor("#A000"));//画笔颜色
	            canvas.drawPaint(paint);  //画画布
	            canvas.restore();//释放画布
	            canvas.drawBitmap(sourceBitmap, matrix, null);//画bitmap
	        }          
	}
}
